...
class TestEcommerceDemo:
    ...
    async def test_homepage_load_and_visual(self):
        ...
        # Capture screenshot
        screenshot_path = await self.browser.capture_screenshot(
        ...
        if Path(baseline_path).exists():
            ...
            assert result['passed'], f"Visual regression detected! Score: {result['overall_score']}"
        else:
            # Create baseline
    ...
    async def test_checkout_flow(self):
        ...
        await self.browser.ai_fill('[data-test="firstName"]', checkout_data['first_name'])
        await self.browser.ai_fill('[data-test="lastName"]', checkout_data['last_name'])
        await self.browser.ai_fill('[data-test="postalCode"]', checkout_data['address']['zip_code'])
    ...
    async def test_performance_audit(self):
        ...
        assert audit_results.get('performance', 0) > 50, \
