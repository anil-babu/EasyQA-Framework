...
try:
    ...
    warnings.warn(
        "scikit-learn/pandas not available. ML features will be limited. "
...
class MLAnalytics:
    ...
    def predict_test_failure(self, test_metadata: Dict) -> Dict:
        ...
        try:
            ...
            prediction = {
                "prediction": (
                    "likely_to_fail" if failure_probability > 0.7 else "likely_to_pass"
                ),
                ...
                "insights": self._generate_prediction_insights(
                    test_metadata, failure_probability
                ),
            }
    ...
    def _extract_features(self, test_data: Dict) -> List[float]:
        ...
        browser_encoded = {"chrome": 0, "firefox": 1, "safari": 2, "edge": 3}.get(
            browser, 0
        )
        ...
        priority_encoded = {"low": 0, "medium": 1, "high": 2, "critical": 3}.get(
            priority, 1
        )
    ...
    def _calculate_recent_failure_rate(self, test_name: str, days: int = 7) -> float:
        ...
        recent_results = [
            ...
            and datetime.fromisoformat(r.get("timestamp", datetime.now().isoformat()))
            > cutoff_date
        ]
    ...
    def _generate_prediction_insights(
        self, test_metadata: Dict, failure_probability: float
    ) -> List[str]:
        ...
        if failure_probability > 0.7:
            ...
            # Check for patterns
            recent_rate = self._calculate_recent_failure_rate(
    ...
    def _get_browser_failure_rate(self, browser: str) -> float:
        ...
        browser_tests = [r for r in self.test_history if r.get("browser") == browser]
    ...
    def analyze_failure_patterns(self) -> Dict:
        ...
        analysis = {
            "total_tests": len(df),
            "failure_rate": (
                len(df[df["status"] == "failed"]) / len(df) if len(df) > 0 else 0
            ),
    ...
    def _get_most_failing_tests(self, df) -> List[Dict]:
        ...
        test_stats = (
            df.groupby("name")
            .agg({"status": lambda x: (x == "failed").sum() / len(x)})
            .reset_index()
        )
    ...
    def _get_failures_by_browser(self, df) -> Dict:
        ...
        return {
            browser: round(rate, 3) for browser, rate in browser_stats["status"].items()
        }
    ...
    def _get_failures_by_time(self, df) -> Dict:
        ...
        return {
            int(hour): round(rate, 3) for hour, rate in time_stats["status"].items()
        }
    ...
    def generate_test_insights(self) -> Dict:
        ...
        if flaky_tests:
            ...
            insights["recommendations"].extend(
                [f"  - {test}" for test in flaky_tests[:5]]
            )
    ...
    def _analyze_trends(self) -> Dict:
        ...
        return {
            "recent_failure_rate": (
                round(recent_trend, 3) if not pd.isna(recent_trend) else 0.0
            ),
